...
from image_folder import ImageFolder_iaa, ImageFolder_iaa_multi_weeather
import numpy as np
import random
...
parser.add_argument('--resume_name',default='two_view', type=str, help='trained model name')
...
#*** debug setup ***
# opt.name='debug'
# opt.data_dir='/home/wangtingyu/datasets/University-Release/train'
# opt.batchsize=8
# opt.stride=1
# opt.views=3
# opt.share=True
...
# opt.resume_name='/home/wangtingyu/LPN-ibn/model/three_view_long_share_d0.75_256_s1_google_lr0.01_ibn/net_199.pth'
...
if opt.iaa:
    ...
    iaa_drone_transform = iaa.Sequential([
        iaa.Resize({"height":opt.h, "width":opt.w}),
        ...
        # iaa.Rain(drop_size=(0.005, 0.015), speed=(0.04, 0.06)),
        # iaa.Rain(drop_size=(0.005, 0.015), speed=(0.04, 0.06))

        # iaa.Snowflakes(flake_size=(0.45, 0.7), speed=(0.005, 0.01)),
        # iaa.Snowflakes(flake_size=(0.2, 0.45), speed=(0.01, 0.03))
...
if opt.iaa:
    ...
    if opt.multi_weather:
        ...
        image_datasets['drone'] = ImageFolder_iaa_multi_weeather(os.path.join(data_dir, 'drone'), transform=transform_iaa_drone_list,
                                                iaa_transform=iaa_drone_transform, iaa_weather_list=iaa_weather_list, batchsize=opt.batchsize)
    else:
        image_datasets['drone'] = ImageFolder_iaa(os.path.join(data_dir, 'drone'), transform=transform_iaa_drone_list,
...
if opt.views == 2:
    ...
    if opt.LPN:
        ...
        model = three_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, norm = opt.norm)

# load params of trained ibn
trained_dict = torch.load(opt.resume_name)
...
if start_epoch>=40:
    opt.lr = opt.lr*0.1
if not opt.LPN:
    # finetuned_params = filter(lambda p: p.requires_grad, model.parameters())
    optimizer_ft = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=opt.lr, weight_decay=5e-4, momentum=0.9, nesterov=True)
    # optimizer_ft = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),
    #                                     lr=opt.lr, betas=(0, 0.999), weight_decay=5e-4)
    '''
    if opt.norm == 'ada-ibn':
        ...
        pt_vgg_params = list(map(id, model.pt_model.model.parameters()))
        pt_mlp_params = filter(lambda p: id(p) not in pt_vgg_params, model.pt_model.parameters())
