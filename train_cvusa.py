...
from image_folder import ImageFolder_iaa, ImageFolder_iaa_multi_weather
...
if opt.iaa:
    print('-----------------using iaa to augment the street image----------------------------')
    iaa_street_transform = iaa.Sequential([
    ...
    transform_iaa_street_list = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
...
if opt.iaa:
    ...
    if opt.multi_weather:
        print('-----------------using multiple weather to augment the street image----------------------------')
        image_datasets['street'] = ImageFolder_iaa_multi_weather(os.path.join(data_dir, 'street'), transform=transform_iaa_street_list,
                                                iaa_transform=iaa_street_transform, iaa_weather_list=iaa_weather_list, batchsize=opt.batchsize, shuffle=True, norm=opt.norm, select=True)
    else:
        image_datasets['street'] = ImageFolder_iaa(os.path.join(data_dir, 'street'), transform=transform_iaa_street_list, iaa_transform=iaa_street_transform)
else:
...
#                print(iter_)
                inputs, labels = data
                if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                    inputs2, labels2, wlabels2 = data2
                    wlabels1 = torch.zeros_like(wlabels2)
                else:
                ...
                if use_gpu:
                    ...
                    if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                        wlabels2 = Variable(wlabels2.cuda().detach())
                ...
                if phase == 'val':
                    ...
                    if opt.views == 2:
                        if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                            outputs, outputs2, sout_w, gout_w = model(inputs, inputs2)
                        else:
                            outputs, outputs2 = model(inputs, inputs2)
                    elif opt.views == 3: 
                        if opt.extra_Google:
                ...
                if not opt.LPN:            
                    ...
                    if opt.views == 2:
                        ...
                        if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                            ...
                            _, g_pred_w = torch.max(gout_w.data, 1)
                            loss_s = criterion(sout_w, wlabels1)
                            loss_g = criterion(gout_w, wlabels2)
                            loss_w = 1 * (loss_g + loss_s)
                            loss = loss + loss_w
                    elif opt.views == 3:
...
#                    print("learning rate is ", optimizer.param_groups[0]["lr"])
                ...
                if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                    ...
                    running_corrects_g_w += float(torch.sum(g_pred_w == wlabels2))
            ...
            if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                ...
                epoch_acc_gw = running_corrects_g_w / dataset_sizes['satellite']
            if opt.views == 2:
                if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                    print('{} Loss: {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} WeatherLoss: {:.4f} W_S_Acc: {:.4f} W_G_Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc, epoch_acc2, epoch_weather_loss,
                                                                                                                                                 epoch_acc_sw, epoch_acc_gw))
                else:
                    print(
                        '{} Loss: {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} '.format(
...
if opt.views == 2:
    if opt.LPN:
        ...
        model = two_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16=opt.use_vgg16, norm=opt.norm, adain=opt.adain, btnk=opt.btnk)
elif opt.views == 3:
